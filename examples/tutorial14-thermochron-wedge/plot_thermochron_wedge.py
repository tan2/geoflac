...
def main():
    ...
    # Create images directory for plots if it doesn't exist
    ...
    # Plot all 4 outer boundaries
    ax1.plot(X_km[:, 0], Z_km[:, 0], color='black', linewidth=1.5)
    ax1.plot(X_km[0, :], Z_km[0, :], color='black', linewidth=1.5)
    ax1.plot(X_km[-1, :], Z_km[-1, :], color='black', linewidth=1.5)
    ax1.plot(X_km[:, -1], Z_km[:, -1], color='black', linewidth=1.5)
    ...
    ax2.plot(X_km[0, :], Z_km[0, :], color='black', linewidth=1.5)
    ax2.plot(X_km[-1, :], Z_km[-1, :], color='black', linewidth=1.5)
    ax2.plot(X_km[:, -1], Z_km[:, -1], color='black', linewidth=1.5)
    ...
    plt.tight_layout()
    plt.savefig('images/thermochron_wedge.png', dpi=300)
    print("Saved 'images/thermochron_wedge.png'")
    
    # Create an evolution plot showing lithological phases
    if nrec >= 3:
        ...
        for idx, f_idx in enumerate(frames_to_plot):
            ...
            im_phase = ax.pcolormesh(x_f, z_f, phase_f, cmap=cmap_phase, vmin=0.5, vmax=19.5, shading='flat')
            
            # Plot all 4 outer boundaries
            ax.plot(x_f[:, 0], z_f[:, 0], color='black', linewidth=1.5)
            ax.plot(x_f[0, :], z_f[0, :], color='black', linewidth=1.5)
            ax.plot(x_f[-1, :], z_f[-1, :], color='black', linewidth=1.5)
            ax.plot(x_f[:, -1], z_f[:, -1], color='black', linewidth=1.5)
            
            ax.set_title(f'Lithological Phase Evolution at t = {t_f:.2f} Myr', fontsize=12, fontweight='bold')
        ...
        fig_evo.colorbar(im_phase, ax=axes_evo.tolist(), orientation='vertical', pad=0.02, shrink=0.6, label='Lithology/Phase ID')
